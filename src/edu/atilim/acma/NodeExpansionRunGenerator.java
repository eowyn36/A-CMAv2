...
public class NodeExpansionRunGenerator implements Runnable {
	...
	public void run() {
		...
		for (Benchmark b : benchmarks) {
			...
			//ABC
			TaskQueue.push(new ConcurrentParallelBeeColony(String.format("%s ABC 20", b.name), rc, design, 50, 20, 7500, 1));
			...
			//MSD
			TaskQueue.push(new ConcurrentHillClimbing(String.format("%s MSD R30 D5", b.name), rc, design, 30, 5, 1));
			TaskQueue.push(new ConcurrentHillClimbing(String.format("%s MSD R30 D10", b.name), rc, design, 30, 10, 1));
			TaskQueue.push(new ConcurrentHillClimbing(String.format("%s MSD R30 D15", b.name), rc, design, 30, 15, 1));
			TaskQueue.push(new ConcurrentHillClimbing(String.format("%s MSD R30 D20", b.name), rc, design, 30, 20, 1));
		}
